...
Classes são modelos para criar objetos, elas possuem atributos e métodos.

Classes podem ser:

- Abstratas: Não podem ser instanciadas, apenas herdadas.
- Concretas: Podem ser instanciadas.
- Seladas: Não podem ser herdadas.

Classe object:

- "Object" é a classe base de TODAS as classes no C#.
- Está localizada no namespace "System".
- Todas as classes, structs, enums e até tipos primitivos herdam (direta ou indiretamente) de Object.
- Por isso, qualquer tipo pode ser tratado como "object".

 PRINCIPAIS MÉTODOS HERDADOS DE Object:

1. Equals(obj)      → Compara dois objetos.
2. GetHashCode()    → Retorna um código hash do objeto (usado em coleções).
3. GetType()        → Retorna o tipo do objeto em tempo de execução.
4. ToString()       → Retorna uma string representando o objeto.
5. MemberwiseClone()→ Cria uma cópia superficial do objeto (apenas para classes).
6. Finalize()       → Executa limpeza antes de o objeto ser coletado pelo GC.
...
public class Pessoa
{
    ...
    // Sobrescrevendo ToString() para mostrar o nome da pessoa
    public override string ToString()
    {
        return $"Pessoa: {Nome}";
    }

    // Sobrescrevendo Equals para comparar pelo nome
    public override bool Equals(object obj)
    {
        if (obj is Pessoa outraPessoa)
        {
            return Nome == outraPessoa.Nome;
        }
        return false;
    }

    // Sobrescrevendo GetHashCode para manter compatibilidade com Equals
...
Console.WriteLine(p1.ToString());        // "Pessoa: Gabriel"
Console.WriteLine(p1.Equals(p2));        // true (mesmo nome)
Console.WriteLine(p1.GetType().Name);    // "Pessoa"
...
--------------------------------
- Uma classe marcada com "sealed" NÃO pode ser herdada.
- Usada para impedir que outras classes derivem dela.
- Pode ser útil por motivos de segurança, desempenho ou design.
...
public sealed class Utilidades
{
    ...
    {
        Console.WriteLine("Classe selada: não pode ser herdada.");
    }
}

// Erro: não é possível herdar de uma classe selada
// public class MinhaClasse : Utilidades { }



MÉTODO SELADO (sealed method)
--------------------------------
- Usado em conjunto com "override".
- Um método selado NÃO pode ser sobrescrito novamente nas classes filhas.
- Geralmente usado quando você permite herança mas quer "travar" um método específico.
...
{
    // Sobrescrevendo e selando o método
    public sealed override void Metodo()
    {
        Console.WriteLine("Método selado: não pode ser sobrescrito de novo.");
    }
}

public class SubDerivada : Derivada
{
    // Erro: não é possível sobrescrever um método selado
...
Em C#, um construtor é um método especial que é executado automaticamente quando um objeto de uma classe é criado.
O objetivo principal é inicializar o objeto — ou seja, definir valores iniciais para suas variáveis e executar qualquer lógica necessária no momento da criação.

Características dos construtores:

-Mesmo nome da classe
-O construtor sempre tem o mesmo nome da classe onde está.
...
Permite criar o objeto já definindo valores no momento da criação.
...
public class Pessoa
{
    ...
    public Pessoa(string nome, int idade)
    {
        Nome = nome;
        Idade = idade;
        Console.WriteLine("Construtor com parâmetros chamado!");
    }
}

// Uso:
Pessoa p2 = new Pessoa("Gabriel", 25);
Console.WriteLine($"{p2.Nome}, {p2.Idade} anos");


Construtor por herança

Esse tipo de construtor é chamado de construtor com parâmetros e utiliza a chamada ao construtor da classe base (base(...)). 
Ele é útil quando a classe derivada (ContaCorrente) precisa inicializar tanto seus próprios campos quanto os campos herdados da classe base (Conta). 
Ao passar argumentos para o construtor da base, você garante que toda a hierarquia de herança seja corretamente inicializada, evitando inconsistências e facilitando a reutilização de código.
...
Um construtor vazio não recebe parâmetros e é usado para inicializar valores padrão.
Ele é chamado automaticamente quando criamos o objeto sem passar argumentos.
...
INTERFACES EM C#
--------------------------------
- Uma interface define um "contrato" que uma classe ou struct deve cumprir.
- Contém apenas a assinatura de métodos, propriedades, eventos ou indexadores.
- Não pode conter implementação de métodos (até C# 7.x). 
  A partir do C# 8, interfaces podem ter implementações "default".
- Não pode ter campos (variáveis).
- Uma classe pode implementar múltiplas interfaces (diferente da herança de classes, que é única).
- Usada para padronizar o comportamento de classes diferentes.

Regras:
1. Tudo declarado em uma interface é PUBLIC por padrão.
2. Métodos NÃO têm corpo (até C# 7.x).
3. Uma interface pode herdar de outra interface.
4. Métodos com corpo não são obrigatórios.
...
// Uso
IAnimal a1 = new Cachorro { Nome = "Rex" };
IAnimal a2 = new Gato { Nome = "Mimi" };
...
INTERFACE HERDANDO OUTRA INTERFACE
--------------------------------
- Uma interface pode herdar de outra e exigir mais métodos/propriedades.
...
public interface IPassaro : IAnimal, IVoador
{
    void BotarOvo();
}

public class Papagaio : IPassaro
{
    public string Nome { get; set; }
    ...
    public void Voar()
    {
        Console.WriteLine("O papagaio está voando!");
    }

    public void BotarOvo()
    {
        Console.WriteLine("O papagaio botou um ovo!");
    }
}
...
Classe abstrata: Tem como objetivo ser exclusivamente um modelo para ser
herdado, portanto não pode ser instanciada.

Classe concreta: Pode ser instanciada e contém implementações completas de seus métodos.

Classe selada: Não pode ser herdada, ou seja, não permite que outras classes a estendam.
